...
void GSM3ShieldV1AccessProvider::ModemConfigurationContinue()
{
	   ...
	   if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	   {
			...
			{ 
				...
				if(theGSM3ShieldV1ModemCore.getPhoneNumber() && (theGSM3ShieldV1ModemCore.getPhoneNumber()[0]!=0)) 
					{
						theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+CPIN="), false);
				...
				else 
					{
						...
						theGSM3ShieldV1ModemCore.genericCommand_rqp(_command_CGREG);
					}
			}
	...
	else if(ct==4)
	{
		...
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp, auxLocate1, auxLocate2))
		{
			if(resp)
			{
				...
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+IFC=1,1"));
			}
			else
			{
	...
	else if(ct==5)
	{
		...
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
		{
			...
			theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+CMGF=1"));
		}
	}
	else if(ct==6)
	{
		...
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
		{
			...
			theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+CLIP=1"));
		}
	}
	else if(ct==7)
	{
		...
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
		{
			...
			theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("ATE0"));
		}
	}
	else if(ct==8)
	{
		...
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
		{
			...
			theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+COLP=1"));
		}
	}
	else if(ct==9)
	{
