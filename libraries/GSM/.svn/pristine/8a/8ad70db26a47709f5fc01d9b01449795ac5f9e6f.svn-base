...
class GSM3ShieldV1ModemCore : public GSM3SoftSerialMgr, public Print
{
	...
	public:
		...
		/** Write PGM
			...
			@return size
		 */
		virtual size_t writePGM(const char PROGMEM str[], bool CR=true);
		...
		/** Generates a generic AT command request from const char PROGMEM buffer
			@param str			Buffer with AT command
			@param addCR		Carriadge return adding automatically
		 */
		void genericCommand_rqp(const char PROGMEM str[], bool addCR=true);
