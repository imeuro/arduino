...
void GSM3ShieldV1DataNetworkProvider::attachGPRSContinue()
{
	...
	{
		//AT+CGATT	
		theGSM3ShieldV1ModemCore.genericCommand_rqp(_command_CGATT,false);
	...
	else if(ct==2)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				//AT+QIFGCNT
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QIFGCNT=0"));
	...
	else if(ct==3)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			if(resp)
			{
				...
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QICSGP=1,\""),false);
				theGSM3ShieldV1ModemCore.print(theGSM3ShieldV1ModemCore.getPhoneNumber());
				theGSM3ShieldV1ModemCore.genericCommand_rqp(_command_SEPARATOR,false);
	...
	else if(ct==4)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			if(resp)
			{
				...
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QIMUX=0"));
	...
	else if(ct==5)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			if(resp)
			{
				...
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QIMODE=1"));
	...
	else if(ct==6)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				// AT+QINDI=1
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QINDI=1"));
	...
	else if(ct==8)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				// AT+QIREGAPP
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QIREGAPP"));
	...
	else if(ct==9)
	{
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			{
				// AT+QIACT	
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QIACT"));
...
void GSM3ShieldV1DataNetworkProvider::getIPContinue()
{
	...
	case 1:
		//AT+QILOCIP
		theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QILOCIP"));
		theGSM3ShieldV1ModemCore.setCommandCounter(2);
		break;
	case 2:
