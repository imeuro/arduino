	...
	case 2:
		...
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp, aux))
	    {
			...
			if(resp)
			{
				...
				// AT+QILPORT
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QILPORT=\"TCP\","),false);
	...
	case 3:
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
	    {
			...
			if(resp)
			{
				...
				// Great. Go for the next step
				// AT+QISERVER
				theGSM3ShieldV1ModemCore.genericCommand_rqp(PSTR("AT+QISERVER"),true);
...
void GSM3ShieldV1ServerProvider::getIPContinue()
{
	...
	case 1:
		//AT+QILOCIP
		theGSM3ShieldV1ModemCore.genericCommand_rqp(_command_MonoQILOCIP);
		theGSM3ShieldV1ModemCore.setCommandCounter(2);
		break;
	case 2:
