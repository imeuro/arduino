...
DateTime::DateTime (uint16_t year, uint8_t month, uint8_t day, uint8_t DoW, uint8_t hour, uint8_t min, uint8_t sec) {
    if (year >= 2000)
        year -= 2000;
    yOff = year;
    m = month;
...
DateTime RTClib::now() {
  ...
  uint8_t w = bcd2bin(Wire.read());
  uint8_t d = bcd2bin(Wire.read());
  uint8_t m = bcd2bin(Wire.read());
  uint16_t y = bcd2bin(Wire.read()) + 2000;
  
  return DateTime (y, m, d, w, hh, mm, ss);
}
