...
void back1hour(){ //do not do this around midnight!!
  int currenthour = Clock.getHour(h12, PM);
  int nexthour = currenthour-1;
  String newTime = GetTime();
  if (currenthour > 0) {
   // cell.Message("OK ho tirato indietro di un ora l`orologio:\n"+newTime);
  } else{
    cell.Message("Aspetta almeno l\'una per settare l\'ora legale!");
  }
